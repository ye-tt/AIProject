...
class DocumentParser(BaseEstimator, TransformerMixin):
...
class KnowledgeBaseIndexer(BaseEstimator, TransformerMixin):
...
class DataProcessingPipeline:
    """Data processing pipeline using sklearn Pipeline."""
    ...
    def _build_pipeline(self) -> Pipeline:
        """Build sklearn pipeline for data processing."""
        pipeline = Pipeline([
    ...
    def process(self, source_dirs: List[Path] | Path) -> dict:
        ...
        logger.info(f"Starting pipeline with {len(source_dirs)} source directories")
