...
class LSTMInputDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        input_tensor = torch.tensor(data_set["review"], dtype=torch.long)
        # BCEWithLogitsLoss target: dtype=torch.float32
        target_tensor = torch.tensor(data_set["label"], dtype=torch.float32)
